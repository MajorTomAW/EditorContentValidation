...
{
	UE_LOG(LogEditorContentValidation, Display, TEXT("Running ContentValidationCommandlet commandlet..."));
	...
	if (P4FilterString && !P4FilterString->IsEmpty())
	{
		...
		{
			UE_LOG(LogEditorContentValidation, Display, TEXT("ContentValidation returning 1. Failed to get changed files."));
			ReturnVal = 1;
		}
	}
	...
	if (bP4Opened)
	{
		...
		{
			UE_LOG(LogEditorContentValidation, Error, TEXT("P4 workspace was not found when using P4Opened"));
			UE_LOG(LogEditorContentValidation, Display, TEXT("ContentValidation returning 1. Workspace not found."));
			ReturnVal = 1;
		}
	}
...
bool UContentValidationCommandlet::GetAllChangedFiles(IAssetRegistry& AssetRegistry, const FString& P4CmdString,
	TArray<FString>& OutChangedPackageNames, TArray<FString>& DeletedPackageNames, TArray<FString>& OutChangedCode,
	TArray<FString>& OutChangedOtherFiles) const
{
	...
	if (LaunchP4(P4CmdString, Results, ReturnCode))
	{
				...
				if (Result.Split(TEXT("#"), &DepotPathName, &ExtraInfoAfterPound))
				{
					if (DepotPathName.EndsWith(TEXT(".uasset")) || DepotPathName.EndsWith(TEXT(".umap")))
					{
						FString FullPackageName;
						{
							...
							if (DepotPathName.Split(TEXT("BroomBroomKartsGame/Content/"), nullptr, &PostContentPath)) //@TODO: RENAME: Potential issue when modules are renamed
							{
						...
						if (FullPackageName.IsEmpty())
						{
							...
							if (DepotPathName.Split(TEXT("BroomBroomKartsGame/Plugins/"), nullptr, &PostPluginsPath))
							{
					...
					{
						FString PostBroomBroomKartsGamePath;
						if (DepotPathName.Split(TEXT("/BroomBroomKartsGame/"), nullptr, &PostBroomBroomKartsGamePath))
						{
							if (DepotPathName.EndsWith(TEXT(".cpp")))
							{
								OutChangedCode.Add(PostBroomBroomKartsGamePath);
							}
							...
							{
								OutChangedOtherFiles.Add(PostBroomBroomKartsGamePath);
							}
						}
					}
				}
		...
		{
			UE_LOG(LogEditorContentValidation, Error, TEXT("p4 returned non-zero return code %d"), ReturnCode);
		}
	}
...
void UContentValidationCommandlet::GetAllPackagesInPath(IAssetRegistry& AssetRegistry, const FString& InPathString,
	TArray<FString>& OutPackageNames) const
{
...
void UContentValidationCommandlet::GetAllPackagesOfType(const FString& OfTypeString,
	TArray<FString>& OutPackageNames) const
{
	...
	for (const FString& Type : Types)
	{
		...
		{
			UE_LOG(LogEditorContentValidation, Error, TEXT("Failed to convert short class name \"%s\" to path name. Please use class path names."), *Type);
		}
		else
		{
...
bool UContentValidationCommandlet::LaunchP4(const FString& Args, TArray<FString>& Output, int32& OutReturnCode) const
{
	...
	{
		UE_LOG(LogEditorContentValidation, Error, TEXT("Failed to launch p4."));
	}
...
FString UContentValidationCommandlet::GetLocalPathFromDepotPath(const FString& DepotPathName) const
{
	...
	if (!Workspace.IsEmpty())
	{
		...
		if (LaunchP4(P4WhereCommand, WhereResults, ReturnCode))
		{
			...
			{
				UE_LOG(LogEditorContentValidation, Warning, TEXT("GetAllChangedFiles failed to run p4 'where'. WhereResults[0] = '%s'. Not adding any validation for %s"), WhereResults.Num() > 0 ? *WhereResults[0] : TEXT("Invalid"), *DepotPathName);
			}
		}
	}
