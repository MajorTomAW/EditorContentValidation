...
TArray<FString> FEditorValidationMessageGatherer::IgnorePatterns;

#define LOCTEXT_NAMESPACE "EditorValidator"


namespace EditorValidatorVariables
{
	static int32 MaxPackagesToLoad = 3000;
	static FAutoConsoleVariableRef CVarEditorValidatorMaxPackagesToLoad
	(
		TEXT("EditorValidator.MaxPackagesToLoad"),
		MaxPackagesToLoad,
		TEXT("The maximum number of packages to load when validating content."),
		ECVF_Default
	);
	...
	(
		...
		TEXT("The maximum number of assets to validate when a single header file changes."),
		ECVF_Default
	);
}
...
void UEditorValidator::ValidateCheckedOutContent(bool bInteractive, const EDataValidationUsecase InValidationUsecase)
{
	...
	if (AssetRegistryModule.Get().IsLoadingAssets())
	{
		if (bInteractive)
		{
			FMessageDialog::Open(EAppMsgType::Ok, LOCTEXT("DiscoveringAssets", "Still discovering assets, please try again later."));
		}
		else
		{
			UE_LOG(LogEditorContentValidation, Display, TEXT("Couldn't run ValidateCheckedOutContent because the asset registry is still loading assets."));
		}
...
bool UEditorValidator::ValidatePackages(const TArray<FString>& ExistingPackageNames,
	const TArray<FString>& DeletedPackageNames, int32 MaxPackagesToLoad, TArray<FString>& OutAllWarningsAndErrors,
	const EDataValidationUsecase InValidationUsecase)
{
	...
	{
		UE_LOG(LogEditorContentValidation, Display, TEXT("Adding referencers for deleted package %s to be verified"), *DeletedPackageName);
		...
		for (const FName& Referencer : PackageReferencers)
		{
			...
			{
				UE_LOG(LogEditorContentValidation, Display, TEXT("    Deleted package referencer %s was added to the queue to be verified"), *ReferencerString);
	...
	if (AllPackagesToValidate.Num() > MaxPackagesToLoad)
	{
		...
		UE_LOG(LogEditorContentValidation, Warning, TEXT("%s"), *WarningMessage);
		...
		if (AssetsToCheck.Num() > 0)
		{
			// Preload all assets to check, so load warnings can be handled separately from validation warnings
			{
				for (const FAssetData& AssetToCheck : AssetsToCheck)
				{
					...
					{
						UE_LOG(LogEditorContentValidation, Display, TEXT("Preloading %s..."), *AssetToCheck.GetObjectPathString());

						// Start listening for load warnings
						FEditorValidationMessageGatherer ScopedPreloadMessageGatherer;
						...
						if (ScopedPreloadMessageGatherer.GetAllWarningsAndErrors().Num() > 0)
						{
							...
							{
								UE_LOG(LogEditorContentValidation, Error, TEXT("%s"), *LoadWarning);
							}
...
bool UEditorValidator::ValidateProjectSettings()
{
...

	{
		...
		if (bDeveloperMode)
		{
			...
			UE_LOG(LogEditorContentValidation, Error, TEXT("%s"), *ErrorMessage);
...
void UEditorValidator::GetChangedAssetsForCode(class IAssetRegistry& AssetRegistry,
	const FString& ChangedHeaderLocalFilename, TArray<FString>& OutChangedPackageNames)
{
	...
	if (Module)
	{
		...
		for (TWeakObjectPtr<UClass> ModifiedClassPtr : ClassList)
		{
			...
			if (UClass* ModifiedClass = ModifiedClassPtr.Get())
			{
				...
				UE_LOG(LogEditorContentValidation, Display, TEXT("Validating Subclasses of %s in %s + %s"), *ModifiedClass->GetName(), *ChangedHeaderModule, *ChangedHeaderReleativeToModule);
				...
				AssetRegistry.EnumerateAssets(Filter, [&BlueprintsDerivedFromNativeModifiedClasses, &bTooManyFiles, &DerivedClassNames, ChangedHeaderModule, ChangedHeaderReleativeToModule](const FAssetData& AssetData)
					{
						...
						if (!AssetData.GetTagValueRef<bool>(FBlueprintTags::IsDataOnly) && !UEditorValidator::IsInUncookedFolder(PackageName))
						{
							...
							if (!ClassFromData.IsEmpty())
							{
								...
								{
									UE_LOG(LogEditorContentValidation, Display, TEXT("\tAdding %s To Validate"), *PackageName);

									BlueprintsDerivedFromNativeModifiedClasses.Emplace(AssetData);

									if (BlueprintsDerivedFromNativeModifiedClasses.Num() >= EditorValidatorVariables::MaxAssetsChangedByAHeader)
									{
										bTooManyFiles = true;
										UE_LOG(LogEditorContentValidation, Display, TEXT("Too many assets invalidated (Max %d) by change to, %s + %s"), EditorValidatorVariables::MaxAssetsChangedByAHeader, *ChangedHeaderModule, *ChangedHeaderReleativeToModule);
